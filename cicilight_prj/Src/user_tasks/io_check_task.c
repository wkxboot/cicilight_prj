...
//汉字命令
  ...
  if(strcmp(cmd_str,MSG_PWR_ON_POSITIVE_PRESSER)==0)
  {
  ...
  if(strcmp(cmd_str,MSG_PWR_ON_NEGATIVE_PRESSER)==0)
  {
   APP_LOG_DEBUG("发送压杯消息！\r\n");
 ...
 if(strcmp(cmd_str,MSG_PWR_ON_POSITIVE_OH_DOOR)==0)
 {
  APP_LOG_DEBUG("发送开升降门消息！\r\n");
  osMessagePut(oh_door_msg_queue_hdl,OH_DOOR_OPEN_MSG,0);
  continue;
 }
  if(strcmp(cmd_str,MSG_PWR_ON_NEGATIVE_OH_DOOR)==0)
  {
  ...
  if(strcmp(cmd_str,MSG_PWR_ON_COMPRESSOR)==0)
  {
  APP_LOG_DEBUG("发送压缩机通电信号！\r\n");
  osMessagePut(compressor_msg_queue_hdl,COMPRESSOR_OPEN_MSG,0);
  continue;
  }
  if(strcmp(cmd_str,MSG_PWR_DWN_COMPRESSOR)==0)
  {
  ...
  if(strcmp(cmd_str,MSG_PWR_ON_JUICING)==0)
  {
  APP_LOG_DEBUG("发送榨汁电机通电消息！\r\n");
  osMessagePut(juice_msg_queue_hdl,JUICE_START_MSG,0);
  continue;
  }
  if(strcmp(cmd_str,MSG_PWR_DWN_JUICING)==0)
  {
  ...
  if(strcmp(cmd_str,MSG_PWR_ON_WHEEL_RGB_LED)==0)
  {
  APP_LOG_DEBUG("发送彩灯旋转消息！\r\n");
  osMessagePut(rgb_led_msg_queue_hdl,RGB_LED_WHEEL_MSG,0);
  continue;
  }
  if(strcmp(cmd_str,MSG_PWR_ON_BLINK_RGB_LED)==0)
  {
  ...
  if(strcmp(cmd_str,MSG_PWR_ON_WHITE_RGB_LED)==0)
  {
    APP_LOG_DEBUG("发送彩灯白色常亮消息！\r\n");
  osMessagePut(rgb_led_msg_queue_hdl,RGB_LED_WHITE_MSG,0);
  continue;
  }
  
  if(strcmp(cmd_str,MSG_PWR_ON_YELLOW_RGB_LED)==0)
  {
  APP_LOG_DEBUG("发送彩灯黄色常亮消息！\r\n");
  osMessagePut(rgb_led_msg_queue_hdl,RGB_LED_YELLOW_MSG,0);
  continue;
  }
  if(strcmp(cmd_str,MSG_PWR_DWN_RGB_LED)==0)
  {
  APP_LOG_DEBUG("发送彩灯断电消息！\r\n");
